...
class Analogues():
    ...
    def getClustering(self):
        ...
        if self.config["model"]["analogues"]["clustering"] == "kMeans":
            return KMeans(n_clusters=nClusters, random_state=randomState).fit(self.predictionMatrix.xTrain)
        
        elif self.config["model"]["analogues"]["clustering"] == "maxDiss":
            if isinstance(self.predictionMatrix.xTrain, pd.DataFrame):
                dataTuple = tuple([self.predictionMatrix.xTrain[col] for col in self.predictionMatrix.xTrain.columns])
            elif isinstance(self.predictionMatrix.xTrain, np.ndarray):
                dataTuple = tuple([self.predictionMatrix.xTrain[:, i] for i in range(self.predictionMatrix.xTrain.shape[1])])
            ...
            return spectralClustering.fit(self.predictionMatrix.xTrain)
    ...
    def getAnalogues(self):
        ...
        # Get the analogues
        if self.config["model"]["analogues"]["clustering"] == "kMeans":
            kNN1 = KNeighborsClassifier(n_neighbors=1).fit(self.predictionMatrix.xTrain, range(len(self.predictionMatrix.xTrain)))
            ...
            for i in range(self.clustering.n_clusters):
                ...
                clusterDataPoints = self.predictionMatrix.xTrain[clusterIndices]
        ...
        return centroids, self.predictionMatrix.xTrain[centroids], self.predictionMatrix.yTrain.iloc[centroids]
    ...
    def getClassifier(self):
        ...
        if self.config["model"]["analogues"]["clustering"] == "kMeans":
            ...
            return classifier.fit(self.predictionMatrix.xTrain, self.clustering.labels_)
